...
def chat_downloader_thread(date, video_id, video_url):
    os.system(
        "chat_downloader --quiet --message_group superchat -o "
        + date
        + "_"
        + video_id
        + ".json "
...
uploads_id = get_channel_uploads_id(youtube_channel_id)
print(uploads_id)

next_page_token = ""
while 1:
    video_ids, next_page_token = get_playlist(
        uploads_id, page_token=next_page_token, max_results=5
    )
    print(video_ids)
    ...
    for video_id in video_ids:
        ...
        # proc = subprocess.Popen(["chat_downloader","--message_group","superchat","-o",date + '[' + video_id +'].csv ', video_info['video_url']]).communicate()

        # print('waiting')

        # os.spawnlp(os.P_DETACH, 'chat_downloader --quiet --message_group superchat -o '+date+'['+video_id+'.csv '+video_info['video_url'])
        td = Thread(
            target=chat_downloader_thread,
            args=[date, video_id, video_info["video_url"]],
        )
        td.start()
        # td.join()
        time.sleep(delay)
        # os.system('chat_downloader --message_group superchat -o ' + date + '_' + video_id +'.csv ' + video_info['video_url'])
