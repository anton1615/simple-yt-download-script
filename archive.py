...
def ytdlp_td():
    os.system("yt-dlp.exe --skip-download --wait-for-video 15 " + url)


def ytarchive_td():
    os.system("ytarchive.exe --wait " + url + " best")


def chatdownloader_txt_td():
    os.system(
        "chat_downloader --message_group all --quiet -o " + filename_txt + " " + url
    )


def chatdownloader_csv_td():
    os.system(
        "chat_downloader --message_group all --quiet -o " + filename_csv + " " + url
    )


def chatdownloader_json_td():
    os.system(
        "chat_downloader --message_group superchat -o " + filename_json + " " + url
    )
...
td2 = Thread(target=chatdownloader_txt_td)
td3 = Thread(target=chatdownloader_csv_td)
td4 = Thread(target=chatdownloader_json_td)
